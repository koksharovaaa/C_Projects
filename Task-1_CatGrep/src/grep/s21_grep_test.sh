...
echo "_OPTIONS -f - e_"
for pattern2 in 123 suppositions "o$" assembly wh gentle "six." terr; do
  TEST1="-f pat.txt $TEST_FILE1 -e $pattern2 $TEST_FILE2"
  echo "$TEST1"
  ./s21_grep $TEST1 >s21_grep.txt
  grep $TEST1 > grep.txt
        DIFF_RES="$(diff -s s21_grep.txt grep.txt)"
...
echo "_OPTIONS -f - e WITH OTHER GREP OPTOINS_"
for options in -i -v -c -l -n -h -s; do
  for options2 in -s -h -n -l -c -v -i; do
    for pattern2 in 123 suppositions WH "o$" assembly gentle "six." terr; do
      TEST1="$options $options2 -f pat.txt $TEST_FILE1 -e $pattern2 $TEST_FILE2"
      echo "$TEST1"
      ./s21_grep $TEST1 >s21_grep.txt
      grep $TEST1 > grep.txt
            DIFF_RES="$(diff -s s21_grep.txt grep.txt)"
